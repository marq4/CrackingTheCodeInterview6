...
def test_all_same():
    ...
    assert linked_lists_are_identical( same_ll, LinkedList( [4, 4, 4, 4] ) )

def test_basic_two_elements():
    """ List has 2 different elements. """
    basic_ll = LinkedList( [1, 2] )
    reverse_function(basic_ll)
    assert linked_lists_are_identical( basic_ll, LinkedList( [2, 1] ) )
